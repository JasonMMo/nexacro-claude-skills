...
description: Nexacro N v24 Form (.xfdl) 작성 헬퍼. Form 골격 + 43종 컴포넌트 블록 + Dataset/BindItem 바인딩 패턴 + 복잡 화면 패턴(Master-Detail 등)을 레퍼런스로 제공합니다. 사용 트리거 — "nexacro 폼 만들어", "xfdl 만들어", "nexacro form maker", "Grid 블록 만들어줘", "Dataset 바인딩", "nexacro component", "xfdl form authoring", "Master-Detail 화면", "상하단 Grid", "FK 관계 화면"
---
...
- 복잡 화면 패턴 (Master-Detail 등) 은 `references/patterns/` 에 분리.
...
## 복잡 화면 패턴 (Complex Screen Patterns)

단순 CRUD 이상의 업무 화면은 아래 패턴 레퍼런스를 참조한다.
각 파일에는 **완성형 XFDL 예제 + Dataset 선언 룰 + gfnTransaction 매핑 + DB→XFDL 변환 룰 + 체크리스트** 포함.

| 패턴 | 사용 조건 | 레퍼런스 |
|------|-----------|----------|
| Master-Detail | DB에 FK 관계 감지 (1:N) — 상단 Grid(목록) + 하단 Grid/Form(상세) | `references/patterns/master-detail.md` |
| Multi-Tab | 동일 화면에서 탭별로 다른 Dataset + Grid (Lazy Loading) | `references/patterns/multi-tab.md` |
| Tree + Grid | 좌측 카테고리 Grid 선택 → 우측 Grid 조회 연동 | `references/patterns/tree-grid.md` |
| Popup 검색 | 팝업 폼에서 조회 후 부모 폼에 값 반환 (opener 패턴) | `references/patterns/popup-search.md` |
| Multi-Dataset 저장 | 헤더(1행 Form) + 라인(Grid) 동시 저장 (입력 전표 구조) | `references/patterns/multi-dataset-save.md` |

→ 패턴 적용 후 반드시 `references/binding-patterns.md`의 Dataset 바인딩 규칙과 교차 확인.
