...
[2/3] 프레임워크 / Framework
      ...
      현재 사용 가능: spring-boot + spring-mvc. 나머지는 upstream
      nexacroN-fullstack 에 placeholder README 만 있으므로 scaffold 시 거부됩니다.
...
3. `framework ∈ {egov-boot, egov-mvc, webflux}` — 업스트림 nexacroN-fullstack 에 placeholder 만 존재 (Plan 2 미구현). 현재는 `spring-boot` (양 lane) + `spring-mvc` (양 lane) 만 scaffold 가능. 사용자에게 alternative 로 `(spring-boot, jdk=8|17)` 또는 `(spring-mvc, jdk=8|17)` 권장.
