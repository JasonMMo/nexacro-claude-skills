...
> **2026-05-07 업데이트 (response dataset name 정렬):** [PR #6](https://github.com/JasonMMo/nexacroN-fullstack/pull/6) 가 머지되어 양 레인 컨트롤러의 응답 dataset 이름이 nxui xfdl 의 `outData` 선언과 일치하도록 정렬되었습니다 — `BoardController` (`/select_data_single.do`, `/select_datalist.do`) → `output1` (xfdl `pattern01-transaction.xfdl`, `pattern02-excel.xfdl`), `LargeController` (`/sampleLargeData.do`) → `ds_largeData` (xfdl `pattern04-largeData.xfdl`), `LoginController` (`/login.do`) → `dsList` (xfdl `frameLogin.xfdl`), `TestDataController` (`/check_testDataTypeList.do`) → `output1` (xfdl `exampleDataType.xfdl`). 이전에는 ad-hoc 이름 (`ds_board`, `ds_large`, `ds_user`, `ds_testData`) 을 사용해 `gfnTransaction` 이 응답 payload 를 로컬 dataset 에 바인드하지 못하던 버그 수정.
